...
size_t movingCountCore(int threshold, size_t rows, size_t cols, size_t i, size_t j, bool* visited);

size_t movingCount(int threshold, size_t rows, size_t cols) {
  bool* visited = new bool[rows*cols];
  memset(visited, 0, rows*cols);
  size_t count = movingCountCore(threshold, rows, cols, 0, 0, visited);
  delete[] visited;
  return count;
}
...
size_t movingCountCore(int threshold, size_t rows, size_t cols, size_t i, size_t j, bool* visited) {
  int count = 0;
  if (0 <= i && i < rows && 0 <= j && j < cols && !visited[i*cols + j] && check(threshold, i, j)) {
