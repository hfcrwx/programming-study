  ...
  for (int i = 0; i < srv.m_conncnt; ++i) {
    ...
    if (sockfd < 0) {
      ...
      log(LOG_INFO,
          __FILE__,
          __LINE__,
          "build connection %d to server success",
          i);
      conn* tmp = NULL;
      try {
        tmp = new conn;
...
conn* mgr::pick_conn(int cltfd) {
  ...
  log(LOG_INFO,
      __FILE__,
      __LINE__,
      "bind client sock %d with server sock %d",
      cltfd,
      srvfd);
  return tmp;
}
